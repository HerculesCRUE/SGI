...
public class RepartoEquipoInventorSpecifications {
  ...
  /**
   * {@link RepartoEquipoInventor} de la entidad {@link InvencionInventor} con el
   * id indicado.
   * 
   * @param id identificador de la entidad {@link InvencionInventor}.
   * @return specification para obtener los {@link RepartoEquipoInventor} de la
   *         entidad {@link InvencionInventor} con el id indicado.
   */
  public static Specification<RepartoEquipoInventor> byInvencionInventorId(Long id) {
    return (root, query, cb) -> cb.equal(root.get(RepartoEquipoInventor_.invencionInventorId), id);
  }
