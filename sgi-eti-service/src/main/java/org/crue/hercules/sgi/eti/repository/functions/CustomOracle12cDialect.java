...
public class CustomOracle12cDialect extends Oracle12cDialect {
  ...
  public CustomOracle12cDialect() {
    ...
    registerFunction("remove_accents",
        new SQLFunctionTemplate(StandardBasicTypes.STRING, "REMOVE_ACCENTS(?1)"));
    registerFunction("remove_html_tags",
        new SQLFunctionTemplate(StandardBasicTypes.STRING, "STRIP_HTML_FUNCTION(?1)"));
    registerFunction("search_in_value_of_json",
        new SQLFunctionTemplate(StandardBasicTypes.STRING, "GET_ALL_JSON_VALUES_FUNCTION(?1)"));
  }
}