...
public abstract class ExecutablePreparedStatementBase implements ExecutablePreparedStatement {
  ...
  protected ExecutablePreparedStatementBase(Database database, String catalogName, String schemaName, String tableName,
   ...
   * @throws SQLException      if JDBC objects to a setting (non-existent bind
   *                           number, wrong column type etc.)
   * @throws DatabaseException if an I/O error occurs during the read of LOB
   *                           values
   */
   ...
   * @param col  a ColumnConfig with information about the column, its type, and
   *             the desired value
   * @throws SQLException      if JDBC objects to a setting (non-existent bind
   *                           number, wrong column type etc.)
   * @throws DatabaseException if an I/O error occurs during the read of LOB
  ...
  /**
   * Gets absolute and normalized path for path. If path is relative, absolute
   * path is calculated relative to change log file.
   *
