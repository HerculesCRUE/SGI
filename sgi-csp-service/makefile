...
makeFullChangeset:
	...
	./mvnw -X clean compile liquibase:diff -Dliquibase.propertyFile=src/main/resources/liquibase-initial.properties -Dliquibase.diffChangeLogFile=src/main/resources/db/changelog/changes/0000000000000-initial-database.xml
	sed -r 's/(\sid=")([0-9]+)(-[0-9]+">)/\10000000000000\3/g' -i src/main/resources/db/changelog/changes/0000000000000-initial-database.xml

makeMigration:
	./mvnw -X compile liquibase:diff -Dliquibase.propertyFile=src/main/resources/liquibase.properties -Dliquibase.diffChangeLogFile=src/main/resources/db/changelog/changes/${DATE_WITH_TIME}-${MIGRATION_LABEL}.xml
