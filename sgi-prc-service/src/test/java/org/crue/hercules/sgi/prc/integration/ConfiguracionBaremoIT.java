...
import org.crue.hercules.sgi.prc.controller.ConfiguracionBaremoController;
...
class ConfiguracionBaremoIT extends BaseIT {
  ...
  @Test
  void findAll_WithPagingSortingAndFiltering_ReturnsConfiguracionBaremoOutputSubList() throws Exception {
    ...
    // when: find ConfiguracionBaremo
    URI uri = UriComponentsBuilder.fromUriString(CONTROLLER_BASE_PATH).queryParam("s", sort).queryParam("q", filter)
        .build(false).toUri();
    final ResponseEntity<List<ConfiguracionBaremoOutput>> response = restTemplate.exchange(uri,
        HttpMethod.GET,
        buildRequest(headers, null, roles),
        new ParameterizedTypeReference<List<ConfiguracionBaremoOutput>>() {
        });

    // given: ConfiguracionBaremo data filtered and sorted
    Assertions.assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);

    final List<ConfiguracionBaremoOutput> responseData = response.getBody();
