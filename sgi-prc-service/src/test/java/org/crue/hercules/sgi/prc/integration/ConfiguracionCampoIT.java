...
import org.crue.hercules.sgi.prc.controller.ConfiguracionCampoController;
...
public class ConfiguracionCampoIT extends BaseIT {
  ...
  @Test
  void findAll_WithPagingSortingAndFiltering_ReturnsConfiguracionCampoOutputSubList() throws Exception {
    ...
    // when: find ConfiguracionCampo
    URI uri = UriComponentsBuilder.fromUriString(CONTROLLER_BASE_PATH).queryParam("s", sort).queryParam("q", filter)
        .build(false).toUri();
    final ResponseEntity<List<ConfiguracionCampoOutput>> response = restTemplate.exchange(uri,
        HttpMethod.GET,
        buildRequest(headers, null, roles),
        new ParameterizedTypeReference<List<ConfiguracionCampoOutput>>() {
        });

    // given: ConfiguracionCampo data filtered and sorted
    Assertions.assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);

    final List<ConfiguracionCampoOutput> responseData = response.getBody();
