...
<html>
  ...
  <body>
    ...
    <p>Siguiendo el calendario de facturación del contrato con la/s empresa/s <b><#list (data.entidadesFinanciadoras)! as entidad>${entidad}<#sep>, </#list></b> titulado <b>${data.tituloProyecto}</b> asociado al/los proyectos con código/s <b><#list data.codigosSge as codigo>${codigo}<#sep>, </#list></b>, se tiene que emitir la ÚLTIMA factura <b><#if data.prorroga>de la prorroga</#if></b>. Para ello es necesario que nos indique si ha hecho entrega del/de la <b>${data.tipoFacturacion}</b> correspondiente.</p>
