...
export class SelectAreasConocimientoPublicTypeComponent extends FieldType implements OnInit {
  ...
  ngOnInit(): void {
    if (!this.value) {
      this.areasConocimiento$ = of([]);
      return;
    }

    this.areasConocimiento$ = this.areaConocimientoService.findById(this.value).pipe(
