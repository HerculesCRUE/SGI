...
@Component({
  selector: 'sgi-config-input-emails',
  templateUrl: './config-input-emails.component.html',
  styleUrls: ['./config-input-emails.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ConfigInputEmailsComponent implements OnInit, OnDestroy {
  ...
  save(): void {
    const newValue = JSON.stringify(this.formGroup.controls.configValue.value);
  ...
  hasChanges(): boolean {
    return this.configValue?.value?.replace(/\s/g, '') !== JSON.stringify(this.formGroup.controls.configValue?.value);
  }
  ...
  private loadConfigValue(key: string): void {
    this.subscriptions.push(
      this.configService.findById(key).subscribe(
        (configValue) => {
          this.configValue = configValue;
          this.formGroup.controls.configValue.setValue(JSON.parse(configValue.value));
        },
        (error) => this.error.next(error)
      )
    );
