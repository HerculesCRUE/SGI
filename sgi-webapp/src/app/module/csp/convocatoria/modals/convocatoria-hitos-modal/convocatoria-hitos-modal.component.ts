...
export class ConvocatoriaHitosModalComponent extends DialogFormComponent<ConvocatoriaHitosModalComponentData> implements OnInit {
  ...
  private fillDefaultAviso(): void {
    ...
    this.configService.getEmailRecipients(DEFAULT_PREFIX_RECIPIENTS_CSP_CONV_HITOS + this.data.unidadGestionId).subscribe(
      (destinatarios) => {
  ...
  private setupI18N(): void {
    ...
    ).subscribe((value) => this.msgParamTipoEntity = {
      entity: value,
  ...
  protected getValue(): ConvocatoriaHitosModalComponentData {
