...
const FACTURAS_JUSTIFICANTES_VIAJES_DIETAS = marker('menu.csp.ejecucion-economica.facturas-justificantes.viajes-dietas');

@Component({
  templateUrl: './viajes-dietas-export-modal.component.html',
  styleUrls: ['./viajes-dietas-export-modal.component.scss']
})
export class ViajesDietasExportModalComponent
  ...
  constructor(
    ...
    viajesDietasExportService: ViajesDietasExportService,
    @Inject(MAT_DIALOG_DATA) private modalData: IDesgloseEconomicoExportData
  ) {
    super(viajesDietasExportService, snackBarService, translate, matDialogRef);
  }
  ...
  protected setupI18N(): void {
    this.title = this.translate.instant(FACTURAS_JUSTIFICANTES) + ' - ' + this.translate.instant(FACTURAS_JUSTIFICANTES_VIAJES_DIETAS);
  }
