import { Component, Inject, OnInit } from '@angular/core';
import { FormControl, FormGroup, Validators } from '@angular/forms';
import { MatDialogRef, MAT_DIALOG_DATA } from '@angular/material/dialog';
import { marker } from '@biesbjerg/ngx-translate-extract-marker';
import { BaseExportModalComponent } from '@core/component/base-export/base-export-modal.component';
import { OutputReport } from '@core/models/rep/output-report.enum';
import { IReportConfig, RelationsTypeView } from '@core/services/rep/abstract-table-export.service';
import { SnackBarService } from '@core/services/snack-bar.service';
import { TranslateService } from '@ngx-translate/core';
import { IEjecucionPresupuestariaReportOptions } from '../../../common/ejecucion-presupuestaria-report-options';
import { IDesgloseEconomicoExportData } from '../../desglose-economico.fragment';
import { DetalleOperacionesIngresosExportService } from './detalle-operaciones-ingresos-export.service';

const DETALLE_OPERACIONES = marker('menu.csp.ejecucion-economica.detalle-operaciones');
const DETALLE_OPERACIONES_INGRESOS = marker('menu.csp.ejecucion-economica.detalle-operaciones.ingresos');

@Component({
  templateUrl: './detalle-operaciones-ingresos-export-modal.component.html',
  styleUrls: ['./detalle-operaciones-ingresos-export-modal.component.scss']
})
export class DetalleOperacionesIngresosExportModalComponent
  extends BaseExportModalComponent<IEjecucionPresupuestariaReportOptions> implements OnInit {

  constructor(
    matDialogRef: MatDialogRef<DetalleOperacionesIngresosExportModalComponent>,
    snackBarService: SnackBarService,
    translate: TranslateService,
    detalleOperacionesIngresosExportService: DetalleOperacionesIngresosExportService,
    @Inject(MAT_DIALOG_DATA) private modalData: IDesgloseEconomicoExportData
  ) {
    super(detalleOperacionesIngresosExportService, snackBarService, translate, matDialogRef);
  }

  ngOnInit(): void {
    super.ngOnInit();
    this.formGroup = this.buildFormGroup();
  }

  protected buildFormGroup(): FormGroup {
    return new FormGroup({
      outputType: new FormControl(this.outputType, Validators.required),
      reportTitle: new FormControl(this.title, Validators.required),
    });
  }

  protected getReportOptions(): IReportConfig<IEjecucionPresupuestariaReportOptions> {
    const reportModalData: IReportConfig<IEjecucionPresupuestariaReportOptions> = {
      title: this.formGroup.controls.reportTitle.value,
      outputType: this.formGroup.controls.outputType.value,
      reportOptions: {
        data: this.modalData.data,
        columns: this.modalData.columns,
        relationsTypeView: this.getRelationsTypeView(this.formGroup.controls.outputType.value),

        columnMinWidth: 120
      }
    };
    return reportModalData;
  }

  protected setupI18N(): void {
    this.title = this.translate.instant(DETALLE_OPERACIONES) + ' - ' + this.translate.instant(DETALLE_OPERACIONES_INGRESOS);
  }

  protected getKey(): string {
    return null;
  }

  protected getGender() {
    return null;
  }


}
