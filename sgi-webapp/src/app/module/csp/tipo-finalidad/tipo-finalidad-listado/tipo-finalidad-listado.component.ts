...
export class TipoFinalidadListadoComponent extends AbstractTablePaginationComponent<ITipoFinalidad> implements OnInit {
  ...
  protected createObservable(reset?: boolean): Observable<SgiRestListResult<ITipoFinalidad>> {
    return this.tipoFinalidadService.findTodos(this.getFindOptions(reset));
  }
  ...
  openModal(tipoFinalidad?: ITipoFinalidad): void {
    const config: MatDialogConfig<ITipoFinalidad> = {
    ...
    dialogRef.afterClosed().subscribe(
      (result: ITipoFinalidad) => {
        if (result) {
          this.snackBarService.showSuccess(tipoFinalidad ? this.textoUpdateSuccess : this.textoCrearSuccess);
          this.loadTable();
        }
      }
    );
