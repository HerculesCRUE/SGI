...
import { ISeguimientoReportData, ISeguimientoReportOptions, RolPersona } from './seguimiento-listado-export.service';
...
@Injectable()
export class SeguimientoEvaluacionesAnterioresListadoExportService extends
  AbstractTableExportFillService<ISeguimientoReportData, ISeguimientoReportOptions>{

  private _tipoComentario: RolPersona;

  public set tipoComentario(value: RolPersona) {
  ...
  public fillColumns(
    convocatoriasData: ISeguimientoReportData[],
  ...
  private getColumnsExcel(convocatoriasData: ISeguimientoReportData[]): ISgiColumnReport[] {
  ...
  public fillRows(
    seguimientos: ISeguimientoReportData[],
    index: number, reportConfig: IReportConfig<ISeguimientoReportOptions>): any[] {

    const seguimientoData = seguimientos[index];
    const elementsRow: any[] = [];
    if (this.isExcelOrCsv(reportConfig.outputType)) {
      const maxNumMemorias = Math.max(...seguimientos.map(evaluacionItem => evaluacionItem.evaluacionesAnteriores ?
        evaluacionItem.evaluacionesAnteriores?.length : 0));
      for (let i = 0; i < maxNumMemorias; i++) {
        const evaluacion = seguimientoData.evaluacionesAnteriores ? seguimientoData.evaluacionesAnteriores[i] ?? null : null;
