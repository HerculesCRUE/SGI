...
@Injectable()
export class EvaluacionEvaluadorResolver extends SgiResolverResolver<IEvaluacionWithComentariosEnviados> {
  ...
  protected resolveEntity(route: ActivatedRouteSnapshot): Observable<IEvaluacionWithComentariosEnviados> {
    ...
    if (this.authService.hasAnyAuthorityForAnyUO(['ETI-EVC-INV-VR', 'ETI-EVC-INV-EVALR'])) {
      return this.service.isEvaluacionEvaluable(Number(route.paramMap.get('id'))).pipe(
        switchMap(response => {
          if (response) {
