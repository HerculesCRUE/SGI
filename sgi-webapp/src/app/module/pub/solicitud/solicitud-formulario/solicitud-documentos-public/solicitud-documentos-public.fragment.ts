...
export class SolicitudDocumentosPublicFragment extends Fragment {
  ...
  constructor(
    ...
    private solicitudPublicId: string,
    ...
    private solicitudDocumentoService: SolicitudDocumentoPublicService,
  ...
  protected onInitialize(): void {
    ...
    const subscription = convocatoriaDocumentoRequeridoSolicitud$.pipe(
      ...
      switchMap((nodes) => {
        return this.solicitudService.findDocumentos(this.solicitudPublicId).pipe(
  ...
  private deleteDocumentos(): Observable<void> {
    ...
    return from(this.documentosEliminados).pipe(
      mergeMap((documento) => {
        return this.solicitudDocumentoService.delete(this.solicitudPublicId, documento.id)
          .pipe(
            tap(() => {
  ...
  private updateDocumentos(nodes: NodeDocumentoSolicitud[]): Observable<void> {
    ...
    return from(nodes).pipe(
      mergeMap((node) => {
        return this.solicitudDocumentoService.update(this.solicitudPublicId, node.documento.value.id, node.documento.value).pipe(
          map((updated) => {
  ...
  private createDocumentos(nodes: NodeDocumentoSolicitud[]): Observable<void> {
    ...
    return from(nodes).pipe(
      mergeMap(node => {
        ...
        return this.solicitudDocumentoService.create(this.solicitudPublicId, node.documento.value).pipe(
          map(created => {
