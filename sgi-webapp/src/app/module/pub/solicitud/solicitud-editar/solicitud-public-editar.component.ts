...
@Component({
  selector: 'sgi-solicitud-public-editar',
  templateUrl: './solicitud-public-editar.component.html',
  styleUrls: ['./solicitud-public-editar.component.scss'],
  viewProviders: [
    SolicitudPublicActionService
  ]
})
export class SolicitudPublicEditarComponent extends ActionComponent implements OnInit {
  ...
  private openCambioEstado(): void {
    const data: SolicitudCambioEstadoPublicModalComponentData = {
      ...
      isTutor: false,
      publicId: this.actionService.publicId
    };
    const config = {
      data
    };
    const dialogRef = this.matDialog.open(CambioEstadoPublicModalComponent, config);
